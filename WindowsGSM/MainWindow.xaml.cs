...
#pragma warning restore
        ...
        private async Task GameServer_Start(ServerTable server, string notes = "")
        {
            ...
            // STARTING visible immédiatement
            _serverMetadata[serverId].ServerStatus = ServerStatus.Starting;
            ...
            try
            {
                ...
                {
                    // passe en updating pour que Discord voie bien l'étape
                    _serverMetadata[serverId].ServerStatus = ServerStatus.Updating;
                    Log(server.ID, "Action: Update | Update on Start");
                    SetServerStatus(server, "Updating");

                    bool updateOk = await GameServer_Update(server, " | Update on Start");

                    // même si update retourne false ou remet stopped, on revient en starting
                    _serverMetadata[serverId].ServerStatus = ServerStatus.Starting;
                    SetServerStatus(server, "Starting");
                }

                // Assure que l'UI montre encore starting juste avant le vrai launch
                _serverMetadata[serverId].ServerStatus = ServerStatus.Starting;
                SetServerStatus(server, "Starting");
                ...
                Log(server.ID, "Server: Started" + notes);
        ...
        {
            if (GetServerMetadata(server.ID).ServerStatus != ServerStatus.Started &&
                GetServerMetadata(server.ID).ServerStatus != ServerStatus.Starting)
            {
                return;
            }
        ...
        {
            if (GetServerMetadata(server.ID).ServerStatus != ServerStatus.Stopped &&
                GetServerMetadata(server.ID).ServerStatus != ServerStatus.Updating)
            {
                return false;
            }
        ...
        private async void OnGameServerExited(ServerTable server)
        {
            ...
            await System.Windows.Application.Current.Dispatcher.Invoke(async () =>
            {
                ...
                if (GetServerMetadata(server.ID).ServerStatus == ServerStatus.Started)
                {
                    ...
                    if (autoRestart)
                    {
                        await RefreshDiscordDynamicMessageAfterStatusChange();
                    }
                    else
                    {
                        await RefreshDiscordDynamicMessageAfterStatusChange(server.ID);
                    }
        ...
        */
        ...
        public async Task RefreshDiscordDynamicMessageAfterStatusChange(string keepStoppedOnlyServerId = null)
        {
            try
            {
                await g_DiscordBot.RefreshDynamicServerListFromLastChannelAsync(keepStoppedOnlyServerId);
            }
            catch (Exception ex)
            {
                Log("System", $"[Discord Dynamic Refresh Error] {ex.Message}");
            }
        }
